...
int ApplicGarageWindow::getIndexModelSelectionCombobox() const
{
    ...
    std::cout<<ui->comboBoxAvailableModels->currentIndex();
...
void ApplicGarageWindow::on_actionNewModel_triggered()
{
    ...
    cout << "image = " << image << endl;
     ...
     if(modelName.empty()) {
        dialogError("Erreur", "Le nom du modèle ne peut pas être vide !");
        return;
    }
    if(power <= 0) {
        dialogError("Erreur", "La puissance doit être un entier positif !");
        return;
    }
    if(engine < 0 || engine > 3) {
        dialogError("Erreur", "Le moteur doit être 0,1,2 ou 3 !");
        return;
    }
    if(basePrice <= 0.0f) {
        dialogError("Erreur", "Le prix de base doit être positif !");
        return;
    }

    carconfig::Engine moteur = carconfig::Engine(engine);

    carconfig::Model newModel = carconfig::Model(modelName.c_str(), power, moteur, basePrice);
    newModel.setImage(image);

    Garage::getInstance().addModel(newModel);
    addAvailableModel(newModel.getName(), newModel.getBasePrice());
}
...
void ApplicGarageWindow::on_actionNewOption_triggered()
{
    ...
    try {
        ...
        carconfig::Option newOption(code, label, price);

        
        Garage::getInstance().addOption(newOption);
...
void ApplicGarageWindow::on_pushButtonSelectModel_clicked()
{
    ...
    if(index == -1)dialogError("pas de Model Selectionne", "Veuillez selectionner un Modele");
    else{
        
        Garage::getCurrentProject().setModel(Garage::getInstance().getModel(index));
        setModel(Garage::getCurrentProject().getModel().getName(), Garage::getCurrentProject().getModel().getPower(), 
                 Garage::getCurrentProject().getModel().getEngine(), Garage::getCurrentProject().getModel().getBasePrice(),
                 Garage::getCurrentProject().getModel().getImage());
...
void ApplicGarageWindow::on_pushButtonAddOption_clicked()
{
    ...
    if(index == -1)dialogError("pas de Model Selectionne", "Veuillez selectionner une Option");
    else{
        try{
        Garage::getCurrentProject().addOption(Garage::getInstance().getOption(index));
        setTableOption(index, Garage::getInstance().getOption(index).getCode(), 
                       Garage::getInstance().getOption(index).getLabel(), Garage::getInstance().getOption(index).getPrice());
        setPrice(Garage::getCurrentProject().getPrice());
    }catch(OptionException& e){dialogError("Erreur Option", e.getMessage().c_str());}
    }
...
void ApplicGarageWindow::on_pushButtonRemoveOption_clicked()
{
    ...
    if(index == -1)dialogError("pas de Model Selectionne", "Veuillez selectionner une Option");
        ...
        Garage::getCurrentProject().removeOption(Garage::getCurrentProject()[index]->getCode());
        setTableOption(index);
        setPrice(Garage::getCurrentProject().getPrice());
...
void ApplicGarageWindow::on_pushButtonReduction_clicked()
{
    ...
    if(index == -1)dialogError("pas de Model Selectionne", "Veuillez selectionner une Option");
        ...
        try{
            (*Garage::getCurrentProject()[index])--;
            setPrice(Garage::getCurrentProject().getPrice());
            setTableOption(index, Garage::getCurrentProject()[index]->getCode(), Garage::getCurrentProject()[index]->getLabel()
                ,Garage::getCurrentProject()[index]->getPrice());
        }catch(OptionException& e){dialogError("Erreur Option", e.getMessage().c_str());}
    }std::cout<<Garage::getCurrentProject()[index]->getPrice()<<std::endl;
}
...
void ApplicGarageWindow::on_pushButtonSaveProject_clicked()
{
    ...
    Garage::getCurrentProject().setName(getCurrentProjectName());
    std::cout<<Garage::getCurrentProject().getName();
...
void ApplicGarageWindow::on_pushButtonOpenProject_clicked()
{
    ...
    Garage::getCurrentProject().load(getCurrentProjectName());


    //mise a jour de table Option
    for (int i=0 ; i<5 ; i++)
    {
        carconfig::Option *ptr = Garage::getCurrentProject()[i];
